...
function safeUser(u) {
  ...
  return {
    ...
    leadsUsed,
    leadsRemaining: Math.max(0, plan.leads - leadsUsed),
...
app.get('/api/places', auth, async (req, res) => {
  ...
  if (remaining <= 0) return res.status(403).json({ error: 'Monthly lead limit reached. Please upgrade your plan.', limitReached: true });
  if (!GOOGLE_KEY) return res.status(500).json({ error: 'Google API key not configured' });
  ...
  try {
    ...
    // Limit grid points by plan
    const maxPts = user.plan === 'free' ? 1 : user.plan === 'starter' ? 3 : points.length;
    const searchPoints = points.slice(0, maxPts);
...
app.get('/api/place-details', auth, async (req, res) => {
  ...
  try {
    ...
    // Deduct 1 lead credit
    const user = getUser(req.user.id);
    if (user) updateUser(user.id, { leadsUsed: (user.leadsUsed || 0) + 1 });
    res.json(r.data);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
