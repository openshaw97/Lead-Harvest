...
// ── AI ENRICHMENT — multi-source, aggressive email finding ──
app.post('/api/enrich', auth, async (req, res) => {
  ...
  try {
    ...
    // Extract domain for pattern-based email guessing
    let domain = null;
    try {
      domain = new URL(url).hostname.replace('www.', '');
    } catch(e) {}

    const prompt = `You are an expert business contact researcher. Your job is to find as many contact details as possible for a business using every available source.
...
Use web search to find contact details from ALL of these sources in order:
1. The business website homepage AND their contact/about page (search: site:${domain || bizName} contact OR about OR email)
2. Their Google Business listing (search: "${bizName}" ${address || ''} email contact)
3. Their Facebook business page if they have one (search: "${bizName}" facebook email)
4. Companies House if it's a UK Ltd company (search: "${bizName}" companies house director email)
5. General web search for their email (search: "${bizName}" email address contact)

EMAIL EXTRACTION RULES:
- Look for any text containing @ on the website
- Check mailto: links carefully
- Look in the footer, contact page, about page, and header
- If you find a contact form but no email, note it
- Try common patterns: info@${domain}, hello@${domain}, contact@${domain}, admin@${domain} - but ONLY include these if you find evidence they're real (e.g. mentioned somewhere)
- Include ALL emails found, not just the first one

Return ONLY a valid JSON object with no other text, markdown, or explanation:
{
  "emails": ["email1@domain.com", "email2@domain.com"],
  "owner_name": "Full Name or null",
  "job_title": "Owner/Director/Manager or null",
  ...
  "description": "One sentence about what this business does"
    ...
    // Validate emails — remove obviously fake ones
    if (result.emails) {
