...
// ── KEYWORD MAP — translate plain English to Google Places types ──
const KEYWORD_MAP = {
  'takeaway': 'meal_takeaway',
  'takeaways': 'meal_takeaway',
  'fish and chips': 'meal_takeaway',
  'chinese takeaway': 'chinese_restaurant',
  'indian takeaway': 'indian_restaurant',
  'pizza': 'meal_takeaway',
  'kebab': 'meal_takeaway',
  ...
  'letting agent': 'real_estate_agency',
  'solicitor': 'lawyer',
...
// ── PLACES — aggressive multi-point grid for maximum results ──
app.get('/api/places', auth, async (req, res) => {
  ...
  try {
    ...
    // Map keyword to Google Places type for better results
    const mappedKeyword = mapKeyword(keyword);

    // Build a tight grid — each cell is 1.5km, overlap guaranteed
    // This ensures we catch everything in dense urban areas
    const CELL = Math.min(searchRadius * 0.4, 1500); // 1.5km cells max
    const SEARCH_R = Math.round(CELL * 1.5); // 50% overlap between cells
    const EARTH = 6371000;
    ...
    for (let dy = -gridSteps; dy <= gridSteps; dy++) {
      for (let dx = -gridSteps; dx <= gridSteps; dx++) {
        ...
        if (dist <= searchRadius) {
          allPoints.push({ lat: clat + dy * latOff, lng: clng + dx * lngOff });
        }
      }
    }

    // All plans get maximum grid coverage — searching is free
    // Just cap to avoid Railway timeout (30s limit)
    const MAX_POINTS = 20;
    const searchPoints = allPoints.slice(0, MAX_POINTS);

    console.log(`Search: "${keyword}" → "${mappedKeyword}" | ${searchPoints.length} grid points | ${SEARCH_R}m radius each`);
    ...
    for (let i = 0; i < searchPoints.length; i++) {
      ...
      for (let p = 0; p < 3; p++) {
        ...
        // Use 'type' for mapped keywords, 'keyword' for plain text
        if (mappedKeyword !== keyword) {
          params.type = mappedKeyword;
        } else {
        ...
        if (resp.data.status === 'REQUEST_DENIED') {
          return res.status(403).json({ error: 'Google API key denied — enable Places API and check billing' });
        }
        if (!['OK', 'ZERO_RESULTS'].includes(resp.data.status)) break;

        for (const pl of (resp.data.results || [])) {
          if (!seen.has(pl.place_id)) {
      ...
      // Small delay between grid points to be respectful to the API
      if (i < searchPoints.length - 1) await new Promise(r => setTimeout(r, 200));
    }
