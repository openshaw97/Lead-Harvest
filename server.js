...
const JWT_SECRET = process.env.JWT_SECRET || 'leadharvest-secret-change-me';
const BASE_URL = process.env.BASE_URL || 'https://lead-harvest-production.up.railway.app';
const MONGODB_URI = process.env.MONGODB_URI;

// ── MONGODB ──
mongoose.connect(MONGODB_URI || 'mongodb://localhost/leadharvest')
...
// ── PLACES — tight grid for maximum results ──
app.get('/api/places', auth, async (req, res) => {
  ...
  try {
    ...
    const CELL = Math.min(searchRadius * 0.4, 1500);
    const SEARCH_R = Math.round(CELL * 1.5);
    const EARTH = 6371000;
    ...
    const searchPoints = allPoints.slice(0, 20);
    const seen = new Set();
    const allResults = [];

    console.log(`"${keyword}"→"${mappedKeyword}" | ${searchPoints.length} pts | r=${SEARCH_R}m`);

    for (let i = 0; i < searchPoints.length; i++) {
      const pt = searchPoints[i];
      let nextToken = null;
      for (let p = 0; p < 3; p++) {
        const params = { location: `${pt.lat},${pt.lng}`, radius: SEARCH_R, key: GOOGLE_KEY };
        if (nextToken) {
          const k = GOOGLE_KEY;
...
app.get('/api/admin/users', async (req, res) => {
  ...
  try {
    ...
    res.json(users.map(u => ({ id: u.id, name: u.name, email: u.email, plan: u.plan, leadsUsed: u.leadsUsed, savedLeads: u.savedLeads.length, createdAt: u.createdAt })));
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`\n✅ LeadHarvest running on port ${PORT}`);
